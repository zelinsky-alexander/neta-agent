...
inline std::unordered_map<std::string, std::int64_t> load_cooldowns(const std::filesystem::path& path) {
    ...
    std::string key; std::int64_t epoch = 0;
...
inline std::uint64_t prevalence(const HistoryStore& store, const ConnectionSummary& target) {
    const auto host = target_host(target);
...
inline FindingAnnouncementInput context_announcement(const ContextRuleMatch& match,
    ...
    finding.host = context.connection.direction == ConnectionDirection::Inbound
        ? context.connection.local_ip : target_host(context.connection);
    finding.port = context.connection.direction == ConnectionDirection::Inbound
        ? context.connection.local_port : context.connection.remote_port;
    finding.transport = "tcp";
    finding.severity = upper(match.severity);
    finding.rule_id = match.rule_id;
    finding.rule_set_id = rules.id;
    finding.rule_set_version = rules.version;
    finding.interpretation = match.interpretation;
    finding.performance_verdict = "INSUFFICIENT_EVIDENCE";
    finding.trust_verdict = "UNVERIFIED";
    finding.evidence_root = context_evidence_root(context);
    const std::string process = !context.connection.process.executable_path.empty()
        ? context.connection.process.executable_path : context.connection.process.comm;
    finding.finding_key = "sha256:" + sha256_hex(
        match.rule_id + "|" + process + "|" + finding.host + ":" + std::to_string(finding.port));
    ...
    finding.changes.emplace_back("Rule: " + match.rule_id);
    finding.changes.emplace_back("Trusted engine: " + match.engine_rule_id);
    finding.changes.emplace_back("Severity: " + finding.severity);
    finding.changes.emplace_back(match.summary);
    return finding;
}
...
inline BehaviorReportingResult auto_report_rule_periodic_behavior(
    ...
    const auto now = rm2_reporting_detail::now_epoch();
    bool changed = false;
    ...
    for (const auto& rule : rules.definitions) {
        ...
        try {
            ...
            auto finding = detect_periodic_outbound(
                store.recent_connections(policy.recent_connection_limit), trigger_connection_id, policy);
            if (!finding) continue;
            ++result.detected;
            finding->type = rule.id;
            finding->severity = rm2_reporting_detail::upper(rule.severity);
            finding->finding_key = "sha256:" + sha256_hex(
                rule.id + "|" + finding->process_identity + "|" + finding->host + ":" + std::to_string(finding->port));
            auto suffix = finding->evidence_root.substr(7);
            if (suffix.size() > 12) suffix.resize(12);
            finding->finding_id = "FINDING-BEHAVIOR-" + rule.id + "-" + suffix;

            if (!rm2_reporting_detail::cooldown_allows(
                    finding->finding_key, cooldowns, now, reporting_policy)) {
                ++result.suppressed_cooldown; continue;
            }
            behavior_reporting_detail::persist_finding(
                behavior_reporting_detail::append_suffix(store.path(), ".findings.jsonl"), *finding, now);
            ++result.persisted;
            cooldowns[finding->finding_key] = now; changed = true;
            ...
            if (reporting_policy.mode == FleetReportingMode::Off ||
                ...
                ++result.suppressed_policy; continue;
            }
            auto announcement = behavior_reporting_detail::announcement_from_finding(*finding);
            announcement.severity = finding->severity;
            announcement.rule_id = rule.id;
            announcement.rule_set_id = rules.id;
            announcement.rule_set_version = rules.version;
            announcement.interpretation = finding->interpretation;
            FleetClient::send_finding(reporting_policy.state_dir, announcement);
            ++result.announced;
        } catch (const std::exception& error) {
            ++result.failed;
            std::cerr << "RM2 periodic rule reporting failed for CONN-" << trigger_connection_id
                      << ": " << error.what() << '\n';
        }
    }
...
inline TransferReportingResult auto_report_rule_large_ingress(
    ...
    for (const auto& rule : rules.definitions) {
        ...
        ++result.considered;
        try {
            LargeIngressPolicy policy;
            policy.minimum_bytes_received = static_cast<std::uint64_t>(rule.numeric("minimum_bytes_received"));
            auto finding = detect_large_ingress(history, transfer_store, connection_id, policy);
            if (!finding) continue;
            ++result.detected;
            finding->type = rule.id;
            finding->severity = rm2_reporting_detail::upper(rule.severity);
            ...
            ++result.failed;
            std::cerr << "RM2 transfer rule reporting failed for CONN-" << connection_id
                      << ": " << error.what() << '\n';
        }
    }
...
inline FleetReportingResult auto_report_context_rules(
    ...
    context.baseline = exported.baseline;
    ...
    if (const auto transfer = transfer_store.latest(connection_id)) {
        context.bytes_sent = transfer->bytes_sent;
        context.bytes_received = transfer->bytes_received;
    }
    ...
    for (const auto& match : matches) {
        ...
        try {
            ...
            if (!rm2_reporting_detail::cooldown_allows(finding.finding_key, cooldowns, now, reporting_policy)) {
                ++result.suppressed_cooldown; continue;
            }
            if (reporting_policy.mode == FleetReportingMode::Off ||
                !std::filesystem::exists(reporting_policy.state_dir / "identity.conf")) {
            ...
            FleetClient::send_finding(reporting_policy.state_dir, finding);
            cooldowns[finding.finding_key] = now; changed = true;
            ++result.announced;
        } catch (const std::exception& error) {
            ++result.failed;
            std::cerr << "RM2 context rule reporting failed for CONN-" << connection_id
                      << ": " << error.what() << '\n';
        }
    }
    if (changed) rm2_reporting_detail::save_cooldowns(rm2_reporting_detail::state_path(store), cooldowns);

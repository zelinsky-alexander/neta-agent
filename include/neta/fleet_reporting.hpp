...
inline std::string with_sha256_prefix(const std::string& hash) {
    if (hash.empty()) return {};
    return hash.starts_with("sha256:") ? hash : "sha256:" + hash;
}

inline std::string normalized_address(std::string address) {
...
inline std::string stable_finding_key(const ExportData& data,
...
inline FleetReportingPolicy fleet_reporting_policy_from_environment() {
    ...
    const auto mode = fleet_reporting_detail::uppercase(
        fleet_reporting_detail::env_value("NETA_FLEET_REPORTING_MODE", "SIGNIFICANT_ONLY"));
    ...
    else if (mode == "SIGNIFICANT" || mode == "SIGNIFICANT_ONLY")
        policy.mode = FleetReportingMode::SignificantOnly;
    else throw std::runtime_error("invalid NETA_FLEET_REPORTING_MODE");

    policy.minimum_confidence = std::stod(
        fleet_reporting_detail::env_value("NETA_FLEET_MIN_CONFIDENCE", "0.80"));
    ...
    policy.cooldown = std::chrono::seconds(std::stoll(
        fleet_reporting_detail::env_value("NETA_FLEET_REPORTING_COOLDOWN_SECONDS", "1800")));
    if (policy.cooldown.count() < 0)
        throw std::runtime_error("NETA_FLEET_REPORTING_COOLDOWN_SECONDS must not be negative");
    policy.state_dir = fleet_reporting_detail::env_value(
        "NETA_FLEET_STATE_DIR", "/var/lib/neta/identity");
    return policy;
}

inline FindingAnnouncementInput finding_from_connection(HistoryStore& store,
    ...
    finding.host = inbound
        ? fleet_reporting_detail::normalized_address(data.connection.local_ip)
        : (!data.connection.target_host.empty()
               ? data.connection.target_host
               : fleet_reporting_detail::normalized_address(data.connection.remote_ip));
    ...
    if (preferred_tls != nullptr) {
        finding.evidence_root = fleet_reporting_detail::with_sha256_prefix(
            tls_session_evidence_hash(*preferred_tls));
    } else if (!data.verdict->input_hash.empty()) {
        finding.evidence_root = fleet_reporting_detail::with_sha256_prefix(data.verdict->input_hash);
    } else {
        throw std::runtime_error("connection has no stable evidence hash to announce");
    }

    finding.finding_key = fleet_reporting_detail::stable_finding_key(data, finding);
...
inline bool finding_matches_reporting_policy(const ExportData& data,
...
inline FleetReportingResult auto_report_connections(
    HistoryStore& store,
    ...
    auto cooldowns = fleet_reporting_detail::load_cooldowns(state_path);
    ...
    if (state_changed) fleet_reporting_detail::save_cooldowns(state_path, cooldowns);
    return result;
}

...
inline std::optional<ContextRuleMatch> evaluate_one(const rules::RuleDefinition& rule,
    ...
    if (engine == "NETA-NET-002") {
        ...
        if (context.metrics.retransmission_delta >= threshold) {
            return match(rule,
                "TCP retransmissions exceeded the configured rule threshold.",
    ...
    if (engine == "NETA-NET-003") {
        if (context.connection.direction != ConnectionDirection::Outbound || context.connection.remote_port == 0)
            return std::nullopt;
        if (!port_allowed(rule, context.connection.remote_port)) {
            return match(rule,
                "Outbound connection used a destination port outside the configured allowlist.",
    ...
    if (engine == "NETA-NET-004") {
        if (context.connection.direction != ConnectionDirection::Outbound || !context.destination_prevalence)
            return std::nullopt;
        const auto maximum = static_cast<std::uint64_t>(rule.numeric("maximum_prevalence"));
        if (*context.destination_prevalence <= maximum) {
            return match(rule,
                "Outbound connection targeted a destination with low local prevalence.",
    ...
    if (engine == "NETA-DNS-001") {
        ...
        if (failed_dns_count(context.name_resolution) >= minimum) {
            return match(rule,
                "Correlated DNS activity contained repeated resolution failures.",
    ...
    if (engine == "NETA-DNS-002") {
        if (context.connection.direction != ConnectionDirection::Outbound || context.name_resolution.empty())
            return std::nullopt;
        if (rule.boolean("require_remote_ip_match") && !dns_contains_remote(context)) {
            return match(rule,
                "Correlated DNS answers did not contain the remote address used by the connection.",
    ...
    if (engine == "NETA-DNS-003") {
        ...
        if (distinct_dns_answers(context.name_resolution) > maximum) {
            return match(rule,
                "Correlated DNS evidence contained more distinct answers than the configured limit.",
    ...
    if (engine == "NETA-TLS-001") {
        ...
        if (rule.boolean("verification_failure_match") && tls->observation.verify_result &&
            *tls->observation.verify_result != 0) bad = true;
        if (bad) {
            return match(rule,
                "Exact application TLS evidence failed the configured validation/authentication policy.",
    ...
    if (engine == "NETA-TLS-002") {
        const auto* tls = exact_outbound_tls(context.tls_sessions);
        if (!tls || !context.baseline) return std::nullopt;
        bool changed = false;
        if (rule.boolean("compare_spki") && !context.baseline->accepted_spki_sha256.empty() &&
            context.baseline->accepted_spki_sha256 != tls->observation.spki_sha256) changed = true;
        if (rule.boolean("compare_issuer") && !context.baseline->accepted_issuer.empty() &&
            context.baseline->accepted_issuer != tls->observation.issuer) changed = true;
        if (changed) {
            return match(rule,
                "TLS peer identity differed from the accepted baseline.",
    ...
    if (engine == "NETA-ROUTE-001") {
        ...
        if (unexpected) {
            return match(rule,
                "Observed route used a gateway or interface outside the configured allowlist.",
...
public:
    ...
    [[nodiscard]] std::vector<ContextRuleMatch> evaluate(const ConnectionRuleContext& context) const {
        ...
        for (const auto& rule : rules_.definitions) {
            if (auto result = context_rule_detail::evaluate_one(rule, context)) {

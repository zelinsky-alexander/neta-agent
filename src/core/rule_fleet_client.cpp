...
ParsedUrl parse_https_url(const std::string& url) {
    ...
    if (!authority.empty() && authority.front() == '[') {
        ...
        if (colon != std::string::npos && authority.find(':') == colon) {
            out.host = authority.substr(0, colon); out.port = authority.substr(colon + 1);
...
std::string decode_chunked(const std::string& body) {
    ...
    for (;;) {
        ...
        const auto extension = size_text.find(';'); if (extension != std::string::npos) size_text.resize(extension);
        std::size_t size = 0;
        try { size = static_cast<std::size_t>(std::stoull(size_text, nullptr, 16)); }
        catch (...) { return body; }
        pos = line_end + 2;
        ...
        if (body.size() - pos < 2 || body.compare(pos, 2, "\r\n") != 0)
            throw std::runtime_error("invalid chunked coordinator response");
        pos += 2;
    }
}
...
std::string header_value(const std::string& headers, const std::string& name) {
    ...
    std::istringstream lines(headers); std::string line;
    while (std::getline(lines, line)) {
        if (!line.empty() && line.back() == '\r') line.pop_back();
        const auto colon = line.find(':'); if (colon == std::string::npos) continue;
        if (lower(line.substr(0, colon + 1)) != needle) continue;
...
std::string authenticated_post(const std::filesystem::path& state_dir, const std::string& path,
                               const std::string& body) {
    const auto identity = FleetClient::load_identity(state_dir);
    const auto url = parse_https_url(identity.coordinator);
    ...
    if (SSL_CTX_set_min_proto_version(context.get(), TLS1_3_VERSION) != 1 ||
        SSL_CTX_set_max_proto_version(context.get(), TLS1_3_VERSION) != 1) ssl_error("TLS 1.3 configuration failed");
    ...
    if (SSL_CTX_use_certificate_chain_file(context.get(), (state_dir / "agent.crt").string().c_str()) != 1)
        ssl_error("cannot load agent certificate");
    ...
    BIO* raw = BIO_new_ssl_connect(context.get()); if (!raw) ssl_error("BIO_new_ssl_connect failed");
    BioPtr bio(raw, BIO_free_all); SSL* ssl = nullptr; BIO_get_ssl(bio.get(), &ssl);
    if (!ssl) ssl_error("TLS BIO has no SSL object"); configure_peer(ssl, url.host);
    ...
    for (;;) {
        ...
        if (count == 0) break; if (BIO_should_retry(bio.get())) continue; break;
    }
    ...
    std::string headers = response.substr(0, header_end + 2); std::string response_body = response.substr(header_end + 4);
...
std::string FleetClient::acknowledge_rule_bundle(const std::filesystem::path& state_dir,
                                                 std::uint64_t revision,
                                                 const std::string& sha256,
                                                 const std::string& status,
                                                 const std::string& error) {
    std::ostringstream body;
    body << "{\"revision\":" << revision << ",\"sha256\":\"" << json_escape(sha256)

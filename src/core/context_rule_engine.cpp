...
std::vector<std::string> correlated_domains(
...
bool port_allowed(const rules::RuleDefinition& rule, std::uint16_t port) {
...
const TlsSessionEvidence* exact_outbound_tls(
...
std::size_t failed_dns_count(const std::vector<NameResolutionEvidence>& evidence) {
    return static_cast<std::size_t>(std::count_if(
        evidence.begin(), evidence.end(), [](const NameResolutionEvidence& item) {
            return item.observation.result_code && *item.observation.result_code != 0;
        }));
}

bool dns_contains_remote(const ConnectionRuleContext& context) {
...
std::size_t distinct_dns_answers(const std::vector<NameResolutionEvidence>& evidence) {
...
ContextRuleMatch match(const rules::RuleDefinition& rule,
                       std::string summary,
                       std::string interpretation) {
    static const std::unordered_map<std::string, std::string> semantic_types{
        {"NET-002", "TCP_RETRANSMISSION_SPIKE"},
        {"NET-003", "UNUSUAL_OUTBOUND_DESTINATION_PORT"},
        {"NET-004", "RARE_OUTBOUND_DESTINATION"},
        {"NET-005", "HIGH_OUTBOUND_ASYMMETRY"},
        {"DNS-001", "REPEATED_DNS_FAILURES"},
        {"DNS-002", "DNS_CONNECTION_MISMATCH"},
        {"DNS-003", "DNS_ANSWER_CHURN"},
        {"TLS-001", "TLS_VALIDATION_FAILURE"},
        {"TLS-002", "TLS_IDENTITY_CHANGE"},
        {"ROUTE-001", "UNEXPECTED_ROUTE"},
    };
    const auto semantic = semantic_types.find(rule.engine_rule_id);
    return ContextRuleMatch{rule.id, rule.engine_rule_id, rule.category, rule.severity,
                            semantic == semantic_types.end() ? rule.engine_rule_id : semantic->second,
                            std::move(summary), std::move(interpretation)};
}

std::optional<ContextRuleMatch> evaluate_one(const rules::RuleDefinition& rule,
    ...
    if (rules::excludes_connection(rule.exclude, context.connection, domains)) {
        return std::nullopt;
    }
    ...
    if (engine == "NET-002") {
        const auto threshold = static_cast<std::uint64_t>(
            rule.numeric("retransmission_threshold"));
    ...
    if (engine == "NET-003") {
        if (context.connection.direction != ConnectionDirection::Outbound ||
            context.connection.remote_port == 0) return std::nullopt;
        if (!port_allowed(rule, context.connection.remote_port)) {
            return match(rule, "Outbound connection used a destination port outside the configured allowlist.",
                "An unusual destination port can be legitimate application behavior or a sign of tunneling or custom protocol use; malicious intent is not established.");
        }
        return std::nullopt;
    }
    if (engine == "NET-004") {
        if (context.connection.direction != ConnectionDirection::Outbound ||
            !context.destination_prevalence) return std::nullopt;
        const auto maximum = static_cast<std::uint64_t>(
            rule.numeric("maximum_prevalence"));
    ...
    if (engine == "NET-005") {
        if (context.connection.direction != ConnectionDirection::Outbound ||
            !context.bytes_sent || !context.bytes_received) return std::nullopt;
        const auto minimum_sent = static_cast<std::uint64_t>(
            rule.numeric("minimum_bytes_sent"));
        const auto minimum_ratio = rule.numeric("minimum_sent_to_received_ratio");
        const auto received_floor = static_cast<std::uint64_t>(
            rule.numeric("received_bytes_floor"));
        if (*context.bytes_sent < minimum_sent) return std::nullopt;
        const auto denominator = std::max(*context.bytes_received, received_floor);
        const auto ratio = static_cast<long double>(*context.bytes_sent) /
                           static_cast<long double>(denominator);
        if (ratio >= static_cast<long double>(minimum_ratio)) {
            return match(rule, "Outbound transfer volume was highly asymmetric.",
                "A high outbound-to-inbound byte ratio can be expected for uploads, backups, or telemetry and can also merit investigation; data exfiltration is not established.");
        }
        return std::nullopt;
    }
    if (engine == "DNS-001") {
    ...
    if (engine == "DNS-002") {
        if (context.connection.direction != ConnectionDirection::Outbound ||
            context.name_resolution.empty()) return std::nullopt;
    ...
    if (engine == "DNS-003") {
        const auto maximum = static_cast<std::size_t>(
            rule.numeric("maximum_distinct_answers"));
    ...
    if (engine == "TLS-001") {
        const auto* tls = exact_outbound_tls(context.tls_sessions);
        if (tls == nullptr) return std::nullopt;
        const bool unauthenticated = rule.boolean("require_peer_authentication") &&
                                     !tls->observation.peer_authenticated;
        const bool verification_failed = rule.boolean("verification_failure_match") &&
            tls->observation.verify_result && *tls->observation.verify_result != 0;
        if (unauthenticated || verification_failed) {
            return match(rule, "Exact application TLS evidence failed the configured validation or authentication policy.",
    ...
    if (engine == "TLS-002") {
        const auto* tls = exact_outbound_tls(context.tls_sessions);
        if (tls == nullptr || !context.baseline) return std::nullopt;
        const bool spki_changed = rule.boolean("compare_spki") &&
            !context.baseline->accepted_spki_sha256.empty() &&
            context.baseline->accepted_spki_sha256 != tls->observation.spki_sha256;
        const bool issuer_changed = rule.boolean("compare_issuer") &&
            !context.baseline->accepted_issuer.empty() &&
            context.baseline->accepted_issuer != tls->observation.issuer;
        if (spki_changed || issuer_changed) {
    ...
    if (engine == "ROUTE-001") {
        ...
        const bool unexpected_gateway =
            !gateways.empty() && !list_contains(gateways, context.route->gateway);
        const bool unexpected_interface = !interfaces.empty() &&
            !list_contains(interfaces, context.route->interface_name);
        if (unexpected_gateway || unexpected_interface) {
            return match(rule, "Observed route used a gateway or interface outside the configured allowlist.",
                "Unexpected route selection can result from VPNs, failover, local reconfiguration, or traffic redirection and should be correlated with host or network changes.");
        }
    }
    return std::nullopt;
}
...
ContextRuleEngine::ContextRuleEngine(const RuleSet& rules) : rules_(rules) {}

std::vector<ContextRuleMatch> ContextRuleEngine::evaluate(
    const ConnectionRuleContext& context) const {
    std::vector<ContextRuleMatch> matches;
    for (const auto& rule : rules_.definitions) {
        if (auto result = evaluate_one(rule, context)) {

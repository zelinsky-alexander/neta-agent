...
void write_file(const std::filesystem::path& path, const std::string& content, mode_t mode) {
    const auto temp = path.string() + ".tmp";
    {
        ...
        out << content;
        if (!out) throw std::runtime_error("failed writing " + temp);
    }
    if (chmod(temp.c_str(), mode) != 0) {
        std::filesystem::remove(temp);
        throw std::runtime_error("chmod failed for " + temp);
    }
...
std::string csr_pem(EVP_PKEY* key, const std::string& display_name) {
    ...
    if (X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC,
                                   ...
                                   -1, -1, 0) != 1)
        ssl_error("CSR subject creation failed");
...
HttpResponse https_post(const std::string& coordinator, const std::filesystem::path& fleet_ca,
    ...
    if (SSL_CTX_load_verify_locations(context.get(), fleet_ca.c_str(), nullptr) != 1)
        ssl_error("cannot load Fleet CA");

    if (client_certificate && client_key) {
        if (SSL_CTX_use_certificate_chain_file(context.get(), client_certificate->c_str()) != 1)
            ssl_error("cannot load agent certificate chain");
        if (SSL_CTX_use_PrivateKey_file(context.get(), client_key->c_str(), SSL_FILETYPE_PEM) != 1)
    ...
    const int fd = connect_tcp(url.host, url.port);
    struct FdGuard { int fd; ~FdGuard() { if (fd >= 0) close(fd); } } fd_guard{fd};
    SslPtr ssl(SSL_new(context.get()), SSL_free);
    if (!ssl) ssl_error("SSL_new failed");
    if (SSL_set_fd(ssl.get(), fd) != 1) ssl_error("SSL_set_fd failed");
    configure_peer_verification(ssl.get(), url.host);
    if (SSL_connect(ssl.get()) != 1) ssl_error("TLS connection failed");
    ...
    while (written < wire.size()) {
        std::size_t count = 0;
        if (SSL_write_ex(ssl.get(), wire.data() + written, wire.size() - written, &count) != 1)
            ssl_error("HTTPS write failed");
        written += count;
    }
    ...
    for (;;) {
        std::size_t count = 0;
        const int rc = SSL_read_ex(ssl.get(), buffer, sizeof(buffer), &count);
        if (rc == 1) {
        ...
        if (error == SSL_ERROR_SYSCALL && count == 0) break;
        ssl_error("HTTPS read failed");
    }
    ...
    if (line_end == std::string::npos) throw std::runtime_error("invalid HTTP status line");
...
std::uint64_t next_sequence(const std::filesystem::path& state_dir) {
    ...
    if (fd < 0) throw std::runtime_error("cannot open sequence lock");
    struct Guard { int fd; ~Guard() { if (fd >= 0) { flock(fd, LOCK_UN); close(fd); } } } guard{fd};
    if (flock(fd, LOCK_EX) != 0) throw std::runtime_error("cannot lock sequence state");
...
FleetIdentity FleetClient::enroll(const FleetEnrollmentOptions& options) {
    ...
    if (chmod(options.state_dir.c_str(), 0700) != 0)
        throw std::runtime_error("cannot protect fleet identity directory");
    ...
    write_file(options.state_dir / "agent.key", key_pem, 0600);
    write_file(options.state_dir / "agent.crt", certificate_chain, 0644);
    write_file(options.state_dir / "fleet-ca.crt", read_file(options.fleet_ca), 0644);
    write_file(options.state_dir / "identity.conf", identity_config(identity), 0600);
...
std::string FleetClient::send_agent_hello(const std::filesystem::path& state_dir) {
    return send_payload(state_dir, "AgentHello",
                        "{\"agent_version\":\"0.1.0\",\"platform\":\"linux\"}");
}
